...
from thisistheone import *  # type: ignore
import pandas as pd
import datetime
import os
import uuid
...
async def login_page():
    ...
    if submit_button:
            if check_credentials(username, password): # type: ignore
                ...
                data = await title_page()  # Await the title_page function
                st.write(data)
                #main
                #asyncio.run(main())
...
""", unsafe_allow_html=True)
    ...
    if choices == '✏️  تعديل المعلومات':
        data = await edit_page()
        return data
    
    if choices == '🔑  استلام وتسليم المركبات':
        data = await handover_vehicle()
        return data
...
async def info_page():
    ...
    st.title("المعلومات")
    try:
        cars = await allcars2() # type: ignore
        ...
        print("First insert failed:", str(e))
        action = 'loged in'
        create_log() # type: ignore
        userlog(action) # type: ignore
        st.markdown(
...
async def add_vehicle(*variables):
     st.markdown(
            """
            <style>
    #         .stApp {
    ...
    #  }
     ...
     if submit_button:
         if any(item == '' for item in variables):
             ...
             await add_execution(*variables) # type: ignore
...
async def edit_page():
    ...
    info_edi = await info_to_edit(chassis_number) # type: ignore
    


#handover a vehicle
async def handover_vehicle(*variables):
    ...
    #  }
        ...
        if handover_button:
            await handover_exe(*variables) # type: ignore
...
async def main():
    ...
    if st.session_state.logged_in:
        #await title_page()
        #data = asyncio.run(title_page())  # This line executes async functions
        #st.write(data)
        data = await title_page()  # Await the title_page function
    else:
        #login_page()
        #data = asyncio.run(login_page())  # Make sure login_page is also async if needed
        #st.write(data)
        data = await login_page()  # Await the login_page if it's async
